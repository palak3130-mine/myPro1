...
def assigned_tickets(tickets):
    ...
    candidates = Member.objects.filter(specialty=tickets.issue_type, is_active=True)
    ...
    candidates = candidates.annotate(
        active_load=Count('assigned_tickets', filter=Q(assigned_tickets__status__in=['Open', 'Assigned']))
    ).order_by('active_load')

    # 3. Pick the winner (lowest load)
    best_member = candidates.first()
